...
$$
但是, 看起來有點不對勁, 爲什麼自變量能這麼變啊? 如果沒有忘記 $M$, $N$ 皆爲**齊次函數**的話, 那麼二元齊次函數的定義爲:
$$f(ax, ay) = a^kf(x,y)$$
其中, $k$ 爲整數. 按此定義, 就有
$$
...
但這兩個線性又不是同一個線性的定義.. @_@ 什麼什麼, 好暈呀. 這裏直接採用[{{< fa "fab" "wikipedia-w">}}線性關係](https://zh.wikipedia.org/wiki/%E7%B7%9A%E6%80%A7%E9%97%9C%E4%BF%82#%E5%AE%9A%E4%B9%89)中的定義:
> 如果稱一個数学函數 $L(x)$ 為線性的，可以是指：
>
> - 定义 1： $L(x)$ 是個只擁有一個變數的一階多項式函數，即是可以表示為 $ L(x)=kx+b $ 的形式（其中 $k,b$ 為常数）.
> - 定义 2： $L(x)$ 具有以下兩個性質：
>   - 可加性: $L(x+t) = L(x)+L(t)$
>   - 一次齊次性: $L(mx) = mL(x)$

顯然線性方程組之「線性」指的是定義 1. 而這種定義是流於形式的, 是不一定滿足可和和一階齊次性的. 或者說, 若將線性方程的自由項置於等號右邊, 則等號左邊方爲定義 1 下的線性函數. (尋常所見的線性方程組俱爲此形式 )
...
$$
\begin{gathered}
   (\bm{x}_1 + \bm{x}_2 , y_1 + y_2) \in S \cr
   k\bm{x}_1 \in S
\end{gathered}
$$
...
$$
\begin{gathered}
   f(\bm{x}_1 + \bm{x}_2) = f(\bm{x}_1) + f(\bm{x}_2)\cr
...
$$
\begin{gathered}
   &(y_1 + y_2)^{(n)} + a_1(x)(y_1 + y_2)^{(n-1)} + \cdots +a_{n-1}(x)(y_1+y_2)' + a_n(x)(y_1+y_2) = b_1(x) + b_2(x) \cr
...
對於線性微分方程的解, 我們總能得出一些通用的結論. 因爲~~懶~~擔心諸位的熒幕寬度與流量, 就先拿二階舉例, 這裏就直接引用同濟版高數.
...
$$y' + P(x)y  = Q(x)$$
前面說了那麼一大堆，一定能很方便的解方程吧！對吧對吧！那麼要怎麼解呢？ 唔.. 還是得用常數變易法.
